<div class="container-fluid py-4">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="<?= $this->url('admin/contacts') ?>" method="get" class="row g-3">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" class="form-control" id="date_from" name="date_from"
                           value="<?= isset($filters['date_from']) ? $this->escape($filters['date_from']) : '' ?>">
                </div>
                <div class="col-md-3">
                    <label for="date_to" class="form-label">Date de fin</label>
                    <input type="date" class="form-control" id="date_to" name="date_to"
                           value="<?= isset($filters['date_to']) ? $this->escape($filters['date_to']) : '' ?>">
                </div>
                <div class="col-md-3">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" id="search" name="search"
                               ...
                               value="<?= isset($filters['search']) ? $this->escape($filters['search']) : '' ?>">
    ...
    <div class="card shadow">
        <div class="card-body">
            <form id="bulk-delete-form" action="<?= $this->url('admin/contacts/bulk-delete') ?>" method="post">
                        ...
                        <?php else: ?>
                            <?php foreach ($contacts as $contact): ?>
                                <tr>
                                    ...
                                    <td><?= $this->formatDate($contact['dateEnvoi'], 'd/m/Y H:i') ?></td>
                                    <td><?= $this->escape($contact['nom']) ?></td>
                                    <td><?= $this->escape($contact['email']) ?></td>
                                    <td><?= $this->escape($contact['sujet'] ?? 'Sans sujet') ?></td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <a href="<?= $this->url('admin/contacts/' . $contact['id']) ?>" class="btn btn-sm btn-outline-primary me-1"
                                               title="Voir">
                                                <i class="bi bi-eye"></i>
                                            </a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('delete-form').action = '<?= $this->url('admin/contacts') ?>/' + contactId + '/delete';
            });
        });
    });
</script>